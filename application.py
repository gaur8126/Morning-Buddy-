...
def temperature_of_city(city:str):
    system_instruction = """
        ...
        Your job is to convert it into a clear, human-fiendly weather update.

        Guidelines:
        1. Always mention the city and country.
        2. Convert temperature from Ketlvin to Celsius (°C), rounded to 1 decimal.
        3. Inclide: current temperature, feels-like temperature, main weather description,
    ...
    response = client.models.generate_content(
        ...
        contents = f"Generate a clear, friendly weather report with temperature in °C, humidity, wind, sunrise/sunset for the {city} and preactical suggestions om what to wear or carry.",
    config = types.GenerateContentConfig(system_instruction = system_instruction,tools=[get_weather])
    )

    return(response.candidates[0].content.parts[0].text)

# FUNCTION TO GET WEATHER PF A CITY. 
def get_weather(city: str):
    """
    Fetches weather for a given city using API 

    args:
    city(str): city name(eg: Agra)
...
def get_news(topic: str):
    """
    Fetches latest news headlines from a API. 
...
def news_summarizer(url):
    response = client.models.generate_content(
        model = "gemini-2.5-flash",
        contents = f"summarize news from the url {url}, don't add sentence like from where the article is , in this article etc. Just give clear and crisp summary."
    )

    return response.text

# FUNCTION TO GET FORCAST OF ENTIRE DAY FOR A CITY AND PLACES TO VISIT FOR A CITY. 
def get_forcasted_weather(city: str):
    """
    LLM TOOL:- fetches forecaste weather and tourist places to visit for the given city. 

    args:
        city(str): city name(eg - Delhi, Agra)
    """
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            contents = f"""
            Provides the detailed weather foreast for {city} on {datetime.date.today()}.
...
def find_local_events(city: str):
    ...
    args:
    city(str): city name (eg- agra, delhi)
...
# print(find_local_events("Delhi"))
    

# FUNCTION FOR SMART PLANNER 
def smart_planner_(city:str):
    prompt=f"""
